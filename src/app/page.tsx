      ...
      <aside className={["fixed left-0 top-0 z-50 flex h-full flex-col justify-between border-r border-[#b7e33d]/25 bg-[#173b29] py-5 text-emerald-50 shadow-2xl shadow-[#173023]/20 transition-all duration-300 ease-out dark:border-white/10 dark:bg-[#0c1a12]", "w-[18.5rem] px-5 -translate-x-full md:translate-x-0", isMenuOpen ? "translate-x-0" : "-translate-x-full", isMenuOpen ? "md:w-[18.5rem] md:px-5" : "md:w-[5.25rem] md:px-2"].join(" ")}>
        <div className="flex h-[calc(100%-110px)] w-full flex-col gap-7">
          <div className="flex h-20 w-full items-center justify-between border-b border-white/10 pb-3">
            {isMenuOpen ? <div className="flex items-center gap-3"><div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-[#b7e33d] text-[#173023] shadow-lg shadow-[#b7e33d]/10"><Music className="h-5 w-5" /></div><div><div className="text-[15px] font-black tracking-[0.16em]">HARMONIQ</div><div className="mt-1 text-[9px] font-bold uppercase tracking-[0.25em] text-emerald-100/55"></div></div></div> : <button onClick={() => setIsMenuOpen(true)} className="mx-auto flex h-11 w-11 items-center justify-center rounded-2xl bg-[#b7e33d] text-[#173023] transition-transform active:scale-95" title="Open sidebar"><Menu className="h-5 w-5" /></button>}
        ...
        <header className="sticky top-0 z-40 border-b border-[var(--line)] bg-[var(--page)]/90 px-5 py-4 backdrop-blur-xl md:px-8">
          <div className="mx-auto flex max-w-[1480px] items-center justify-between gap-4">
            <div className="flex min-w-0 items-center gap-3">
              ...
              <div className="min-w-0"><div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.24em] text-[var(--muted)]"><AudioWaveform className="h-3.5 w-3.5 text-[var(--accent-strong)]" /> </div><div className="mt-0.5 truncate text-sm font-semibold text-[var(--ink)]">{currentTitle || (activeTab === "settings" ? "Workspace settings" : "New score")}</div></div>
            </div>
            ...
            {activeTab === 'dashboard' && (
              status === 'completed' && currentSheetMusic ? (
                <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
                  <div className="mb-7 flex flex-col gap-5 border-b border-[var(--line)] pb-6 lg:flex-row lg:items-end lg:justify-between"><div className="min-w-0"><div className="mb-3 flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.22em] text-[#2d7b4e] dark:text-[#b7e33d]"><Sparkles className="h-3.5 w-3.5" /> Transcription complete</div><h1 className="truncate display-type text-3xl font-black tracking-[-0.055em] text-[var(--ink)] md:text-5xl">{currentTitle || "Notation output"}</h1><p className="mt-2 flex items-center gap-2 text-sm text-[var(--muted)]"><CircleCheck className="h-4 w-4 text-[#2d7b4e]" /> </p></div><div className="flex flex-wrap items-center gap-2"><button onClick={handleDownloadPdf} className="inline-flex items-center gap-2 rounded-xl bg-[#173b29] px-4 py-2.5 text-sm font-semibold text-[#e9f4c6] shadow-lg shadow-[#173023]/15 transition hover:bg-[#2d7b4e] active:scale-[0.98] dark:bg-[#b7e33d] dark:text-[#173023]" title="Download sheet music as PDF"><Download className="h-4 w-4" /> PDF</button><button onClick={handleDownloadMidi} disabled={!currentMidiBase64 && !currentMidiUrl} className="inline-flex items-center gap-2 rounded-xl border border-[var(--line-strong)] bg-[var(--paper)] px-4 py-2.5 text-sm font-semibold text-[var(--ink)] transition hover:border-[#2d7b4e] hover:bg-[#e9f4c6] disabled:cursor-not-allowed disabled:opacity-40 dark:hover:bg-[#b7e33d]/10" title={currentMidiBase64 || currentMidiUrl ? "Download MIDI file" : "MIDI not available"}><FileMusic className="h-4 w-4" /> MIDI</button><button onClick={handleResetWorkspace} className="rounded-xl border border-[var(--line)] bg-[var(--paper)] p-2.5 text-[var(--muted)] transition hover:border-[var(--line-strong)] hover:text-[var(--ink)]" title="Close result" aria-label="Close result"><X className="h-5 w-5" /></button></div></div>
                  <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_270px]"><div className="paper-grain min-w-0 rounded-[26px] border border-[var(--line)] bg-[var(--paper)] p-3 shadow-[0_24px_70px_rgba(23,48,35,0.10)] md:p-5"><div className="mb-3 flex items-center justify-between px-1 text-[10px] font-bold uppercase tracking-[0.2em] text-[var(--muted)]"><span>Score preview</span><span className="text-[#2d7b4e] dark:text-[#b7e33d]">MusicXML render</span></div><div className="min-h-[560px] overflow-hidden rounded-2xl border border-[var(--line)] bg-[#f4f4ed] p-2 shadow-inner md:p-4"><SheetMusicViewer musicXmlData={currentSheetMusic} /></div></div></div>
                </div>
